...
ELW576_PATH = FIXTURES / "ELW576.rtf"
...
# ── ELW 576: TextEdit/Cocoa-style RTF using \<newline> for breaks ──


class TestTextEditStyle:
    """ELW 576 — We All Are One in Mission.

    This RTF was generated by TextEdit/Cocoa and uses backslash-newline
    instead of \\par for line breaks within stanzas. Per the RTF 1.9.1
    spec, \\<CR>/\\<LF> is equivalent to \\par.
    """

    @pytest.fixture(autouse=True)
    def parse(self):
        self.lyrics = parse_rtf_lyrics(_read(ELW576_PATH), hymn_number="576")

    def test_title(self):
        assert self.lyrics.title == "We All Are One in Mission"

    def test_verse_count(self):
        assert len(self.lyrics.verses) == 3
    ...
    def test_verses_are_numbered(self):
        for i, v in enumerate(self.lyrics.verses, 1):
            assert v.startswith(f"{i}\t")

    def test_first_verse_content(self):
        assert "We all are one in mission" in self.lyrics.verses[0]
        assert "Christ, the Lord of all" in self.lyrics.verses[0]
